...
use hal_wire::{operations::Code, AsCborValue, CborError};
use log::error;
use std::ffi::CString;
use std::fmt::Debug;
...
/// Abstraction of a HAL service that uses an underlying [`SerializedChannel`] to
...
pub trait ChannelHalService<T: SerializedChannel> {
    ...
    fn execute_req<O, R, S>(&self, req: R) -> binder::Result<S>
    where
        O: Debug,
        R: AsCborValue + Code<O>,
        S: AsCborValue,
    {
        let code = req.code();
        ...
        S::from_slice(&rsp_data).map_err(failed_cbor)
    }
}
